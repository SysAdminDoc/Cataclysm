...
function reportText(report: PointProbeReport, label: string, t: ReturnType<typeof useI18n>["t"]): string {
  return [
    label,
    t("probe.export.domain", { value: report.domain }),
    t("probe.export.coordinate", { lat: report.lat.toFixed(6), lon: report.lon.toFixed(6) }),
    t("probe.export.range", { value: report.rangeM.toFixed(1) }),
    ...
    ...report.assumptions.map((assumption) => t("probe.export.assumption", { value: assumption })),
    ...report.unknowns.map((unknown) => t("probe.export.unknown", { value: unknown })),
  ].join("\n");
}
...
export function PointProbePanel({ primary, comparison = null }: Props) {
  ...
  const exportText = () => {
    const body = reports.map(([label, report]) => reportText(report, label, t)).join("\n\n");
    ...
    <section className="section" aria-labelledby="point-probe-heading">
      ...
      {reports.map(([label, report]) => (
        <div className="result-card" key={label}>
          ...
          <p>{report.status} · {t("probe.distance", { value: formatNumber(report.rangeM / 1_000, { minimumFractionDigits: 1, maximumFractionDigits: 1 }) })}</p>
