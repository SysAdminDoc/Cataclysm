...
emergency-response facilities from OpenStreetMap. The layer is off by default.
...
The same Layers view is the scenario's render controller. Use the checkbox to
show or hide an available layer, the slider to change opacity, and the arrow
buttons to change render priority (higher rows draw above lower analytical
overlays). **Legend and provenance** explains the palette, time coupling, model
basis, and prerequisite when data are not yet available. **Reset** restores the
domain defaults. The stack persists locally per scenario, and export metadata
records every layer's visibility, opacity percentage, and order so a shared
image or scientific file describes the view that produced it.
