...
- **Four-language foundation** — Settings persists English, Spanish, Japanese,
  ...
  complete five-source Custom scenario editor and numeric validation, SWE
  solver/recovery/gauge controls, persistent playback, Results and coastal
...
than becoming a bundled Fixed Version runtime. In the current v0.14.1 local
build, the standard MSI/NSIS are about 15/12 MiB and their offline counterparts
are about 209/208 MiB.

The v0.14.1 Windows installers are locally built from this repository and
...
```powershell
# PowerShell
(Get-FileHash .\Cataclysm_0.14.1_x64_en-US.msi -Algorithm SHA256).Hash
```

```cmd
:: Command Prompt
certutil -hashfile Cataclysm_0.14.1_x64_en-US.msi SHA256
```
