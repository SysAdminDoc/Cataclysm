...
- Extended max-field products: every accepted solver step now also accumulates
  ...
  peak amplitude, time-of-max, arrival, and energy fields. All five are computed
  per accepted step (regression-tested) and serialized in the `MaxFieldProduct`
  IPC payload. (Surfacing them in NetCDF/Zarr exports and the Inspect/Results
  panels is tracked in ROADMAP.)
...
- Ground-zero labels now render as explicit Cesium overlays above translucent
  ...
  depth-order and Windows font-rasterization drift from release frames.
...
- The deterministic reference-capture harness now submits every local-only
  workflow control without attaching Cesium requests to Playwright's navigation
  wait, and it waits for direct-render recordings to decode before
