...
NM.EmergencyGuide = {
  generate(det) {
    ...
    const t = NM.t || (key => key);
    const items = [
      {title:t('guide.immediate.title'), body:t('guide.immediate.body'), color:'var(--red)', always:true},
      {title:t('guide.cover.title'), body:t('guide.cover.body'), color:'var(--peach)', always:true},
      {title:t('guide.down.title'), body:t('guide.down.body'), color:'var(--yellow)', always:true},
      {title:t('guide.move.title'), body:t('guide.move.body'), color:'var(--teal)', always:true},
      {title:t('guide.shelter.title'), body:hasFallout ? t('guide.shelter.fallout') : t('guide.shelter.airburst'), color:'var(--blue)', always:true},
      {title:t('guide.stay.title'), body:hasFallout ? t('guide.stay.fallout') : t('guide.stay.airburst'), color:'var(--mauve)', always:hasFallout},
      {title:t('guide.supplies.title'), body:t('guide.supplies.body'), color:'var(--green)', always:true},
    ];
    ...
    if (hasFallout) {
      html += `<div class="guide-item" style="border-left-color:var(--overlay0);margin-top:12px"><div class="gi-title" style="color:var(--overlay0)">${t('guide.fema.title')}</div><div class="gi-body" style="font-size:9px;color:var(--overlay0)">${t('guide.fema.body')}</div></div>`;
